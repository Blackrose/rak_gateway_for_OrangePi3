...
function echo_model_info()
{
    ...
    echo_yellow "*\t3.RAK2247"
    echo_yellow  "Please enter 1-3 to select the model:\c"
}
...
function do_set_model()
{
    ...
    do
        ...
        if [ $RET -eq 0 ]; then
            if [ $RAK_MODEL -lt 1 ] || [ $RAK_MODEL -gt 3 ]; then
                echo_yellow "IF Please enter 1-3 to select the model:\c"
                continue
            else
